...
type FindDoctorDirectoryProps = {
  routes: FindDoctorRoutes;
  /** When true, always show Book (patient portal). When false, only for logged-in patients. */
  alwaysShowBook?: boolean;
  className?: string;
};
...
export function FindDoctorDirectory({ routes, alwaysShowBook = false, className }: FindDoctorDirectoryProps) {
  ...
  const showBook = alwaysShowBook || user?.role === 'PATIENT';
            ...
            <ul className="mt-4 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
              {directory.data.items.map((p: PractitionerDirectoryItem) => {
                ...
                return (
                  <li
                    key={id}
                    className="flex flex-col rounded-2xl border border-[#e2e8f0] bg-white p-5 shadow-[0_8px_28px_-16px_rgba(14,22,61,0.12)] ring-1 ring-black/[0.03] transition hover:shadow-[0_14px_36px_-18px_rgba(14,22,61,0.16)]"
                  >
                    <div className="flex gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="truncate font-semibold text-[#0a1628]">{name}</p>
                        <p className="mt-0.5 flex flex-wrap items-center gap-x-1.5 text-xs text-[#64748b]">
                        ...
                        {locLine ? (
                          <p className="mt-1 flex items-start gap-1 text-xs text-[#64748b]">
                    ...
                    <div className="mt-5 flex gap-2">
                      <Button
                        asChild
                        className="flex-1 rounded-xl border border-slate-200 bg-white text-[13px] font-semibold text-[#0a1628] shadow-sm hover:bg-slate-50"
                        variant="outline"
                        size="sm"
                      >
