...
export function MarketingFooterSocial() {
  return (
    <div className="mt-5 flex flex-wrap gap-2.5">
      {social.map(({ label, href, children }) => (
        <a
          key={label}
          href={href}
          target="_blank"
          ...
          aria-label={label}
          className="flex size-9 items-center justify-center rounded-full bg-brand-navy text-white shadow-sm ring-1 ring-brand-navy/20 transition hover:bg-brand-hero-blue hover:ring-brand-hero-blue/30"
        >
          <svg className="size-[1.05rem]" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
            {children}
          </svg>
        </a>
      ))}
