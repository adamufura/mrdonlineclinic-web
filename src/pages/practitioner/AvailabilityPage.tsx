...
export default function PractitionerAvailabilityPage() {
  ...
  return (
    <>
      <Helmet>
        <title>{t('practitioner.availability.title')}</title>
        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <h1 className="font-display text-2xl font-semibold tracking-tight text-[#0a1628] sm:text-3xl">
              {t('practitioner.availability.heading')}
            </h1>
            <p className="mt-2 max-w-2xl text-sm text-[#64748b]">{t('practitioner.availability.description')}</p>
          </div>
          <Button
            type="button"
          ...
          >
            {t('practitioner.availability.manageSlots')}
          </Button>
        </div>
